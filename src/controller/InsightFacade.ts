...
//TODO: Uncomment this
        ...
        for (let x in finalReturnInfo) {
            ...
            if (Number(x) == 0) {
                ...
                if (newTransformGroup.length == 0) {
                    ...
                    if(groupObjectArray.length > 4900){
                        return groupObjectArray;
                    }
                    //console.time("match Index")
                    var tempString
                    var tempBool;
                    for(let x in groupObjectArray){

                        if(
                      (newTransformGroup.every(function (singleGroup: any) {
                          //tempString = JSON.stringify(singleGroup)
                          newValue = (transformReturnInfo[singleGroup]);
                          return newValue == (groupObjectArray[x][singleGroup]);
                    ...
                    /**matchIndex = groupObjectArray.findIndex(function (singleGroupedElement: any) {
                        return newTransformGroup.every(function (singleGroup: any) {
                            tempString = JSON.stringify(singleGroup)
                            newValue = tempString.concat(JSON.stringify(transformReturnInfo[singleGroup]));
                            return newValue == tempString.concat(JSON.stringify(singleGroupedElement[singleGroup]));

                        })
                    });*/
                    //console.timeEnd("match Index")
                }


                if (Number(matchIndex) >= 0) {
                    transformReturnInfo = Object.assign({}, transformReturnInfo, groupedApplyColumns)
                    //console.time("group merge")
                    groupObjectArray[matchIndex] = this.groupQueryHelper(groupObjectArray[matchIndex], transformReturnInfo, transformationApply)
                    //console.timeEnd("group merge")
    ...
    groupQueryHelper(returnInfo:any, groupReturnInfo:any, transformationApply:any){
        ...
        for(let x in transformationApply){
            ...
            groupTarget = groupReturnInfo[applyKey];
            givenTarget = returnInfo[applyKey];
            //console.time("assert equal")
            if(groupTarget[0] == givenTarget[0]){
                givenTarget.shift();
            }
            //console.timeEnd("assert equal")

            //console.time("update")
            groupReturnInfo[applyKey] = groupTarget.concat(givenTarget)
